...
Publicação v4 confirmada em `https://side-of-death.vercel.app/?v4=1`: o domínio serviu `chapters.js` versão 4.0.0 e `side-of-death-v9`; o modo normal não expôs `window.SOD_DEBUG`, PLAY abriu `CENA 01 · O CEMITÉRIO` com legenda, `#cinematic.show` e `document.fullscreenElement: false`. O deployment Vercel do commit `3b63325` retornou `success` em `https://side-of-death-d1sda7no5-jeanvicens-projects.vercel.app`.
Pacote nativo v4.0.0 gerado após `npm run build` e `npx cap sync android`: `android/app/build/outputs/bundle/release/app-release.aab`, 115 MB, SHA-256 `dd25b1eae49ee72620e63e6d34f32b382d0e9f7aa6bfa31807d94f05c6bc6bed`.
